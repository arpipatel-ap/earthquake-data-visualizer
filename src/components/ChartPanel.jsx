...
const ChartPanel = ({
  ...
  const numericAxes = Object.keys(data[0] || {}).filter(key =>
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="p-2 bg-white border border-gray-300 shadow-md rounded">
          ...
          <p>{selectedXAxis}: {pointData[selectedXAxis]}</p>
          <p>{selectedYAxis}: {pointData[selectedYAxis]}</p>
        </div>
      );
    }
    return null;
    ...
    <div className="p-4 bg-white rounded-lg shadow-md flex flex-col h-full">
      ...
      <div className="flex-grow">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            <CartesianGrid />
            <XAxis type="number" dataKey={selectedXAxis} name={selectedXAxis.charAt(0).toUpperCase() + selectedXAxis.slice(1)} />
            <YAxis type="number" dataKey={selectedYAxis} name={selectedYAxis.charAt(0).toUpperCase() + selectedYAxis.slice(1)} />
            <ZAxis dataKey="mag" range={[60, 400]} name="Magnitude" /> {/* Z-axis for size, based on magnitude */}
            ...
            >
              {data.map((entry) => (
                <Dot
                  ...
                  fill={selectedEarthquakeId === entry.id ? '#FF0000' : '#8884d8'} // Change color if selected
                  stroke={selectedEarthquakeId === entry.id ? '#FF0000' : 'none'}
